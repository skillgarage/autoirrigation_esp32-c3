...
static void soil_sensor_task(void *arg){
    while(1){
        ...
        ESP_LOGI(TAG, "Soil raw = %d, Limit = %d", soil_raw_value, SOIL_WET);
        oled_display_show_values(soil_raw_value, SOIL_WET);
...
static void pump_task(void *arg){
    ...
    while(1){
        ...
        // If pump stopped, check for start conditions
            ...
            else if(soil_raw_value > SOIL_WET){ // if no waiting, check sensor
                water_pump_set(true);
                pump_running = true;
                pump_started = now;
